...
#Remove files if it already exists

folder = Path('./savedLightFmData')
#savedDataset = folder / 'dataset.pkl'
#savedInteractions = folder / 'interactions.npz'
#savedModel = folder / 'lightfm_model.pkl'
#savedDishes = folder / 'dishes.pkl'
#savedDishesMeta = folder / 'dishesMetadata.pkl'

filenames = ['dataset.pkl', 'interactions.npz', 'lightfm_model.pkl', 'dishes.pkl', 'dishesMetadata.pkl']
...
for name in filenames:
    ...
    if file.exists():
        ...
        print(f"{file.name.name} not found.")


#print("savedDataset is: ", savedDataset)

#if savedDataset.exists():
#    savedDataset.unlink()
#    print(f"{savedDataset.name} removed.")
#else:
#    print(f"{savedDataset.name} not found.")
...
# Save dataset
with open('./savedLightFmData/dataset.pkl', 'wb') as f:
    pickle.dump(dataset, f)

# Save interaction matrix
save_npz('./savedLightFmData/interactions.npz', interaction_matrix)

# Save trained model
joblib.dump(model, './savedLightFmData/lightfm_model.pkl')
print("Training complete and saved.")

# Save the dishes variable 
with open('./savedLightFmData/dishes.pkl', 'wb') as f: 
    pickle.dump(dishes, f)

# Save the dishes metadata
with open('./savedLightFmData/dishesMetadata.pkl', 'wb') as f:
